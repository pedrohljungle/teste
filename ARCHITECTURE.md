...
- [1. O paradigma: Hexagonal (Ports & Adapters)](#1-o-paradigma-hexagonal-ports--adapters)
  - [`interfaces/`: os contratos moram fora das implementações](#interfaces-os-contratos-moram-fora-das-implementações)
  - [Middleware: em `libs/`, nomeado pelo que exige, declarado na rota](#middleware-em-libs-nomeado-pelo-que-exige-declarado-na-rota)
  - [`libs/`: onde vai o que não é camada](#libs-onde-vai-o-que-não-é-camada)
  - [Um módulo por domínio dentro de cada camada](#um-módulo-por-domínio-dentro-de-cada-camada)
- [2. Lógica de negócio: carteira, aposta e reversão](#2-lógica-de-negócio-carteira-aposta-e-reversão)
  - [2.1 As seis operações](#21-as-seis-operações)
  - [2.2 Dinheiro: `BIGINT` em unidades mínimas](#22-dinheiro-bigint-em-unidades-mínimas)
  - [2.3 Máquina de estados](#23-máquina-de-estados)
  - [2.4 Referências e reversões](#24-referências-e-reversões)
  - [2.5 Códigos de falha](#25-códigos-de-falha)
  - [2.6 Idempotência](#26-idempotência)
  - [2.7 Concorrência: lock pessimista por linha de carteira](#27-concorrência-lock-pessimista-por-linha-de-carteira)
  - [2.8 Eventos](#28-eventos)
  - [2.9 Observabilidade do domínio](#29-observabilidade-do-domínio)
- [3. Modelo de dados](#3-modelo-de-dados)
  - [3.1 Diagrama](#31-diagrama)
  - [3.2 DDL](#32-ddl)
  - [3.3 O critério: o que é do banco e o que é do domínio](#33-o-critério-o-que-é-do-banco-e-o-que-é-do-domínio)
  - [O que **não** vai para o banco](#o-que-não-vai-para-o-banco)
- [4. Fluxo de uma operação](#4-fluxo-de-uma-operação)
  - [4.1 O que o domínio muda no desenho](#41-o-que-o-domínio-muda-no-desenho)
  - [4.2 Componentes](#42-componentes)
  - [4.3 Estrutura de pastas do domínio](#43-estrutura-de-pastas-do-domínio)
  - [4.4 Onde cada regra mora](#44-onde-cada-regra-mora)
  - [4.5 A transação SQL](#45-a-transação-sql)
  - [4.6 Outbox](#46-outbox)
  - [4.7 Filas](#47-filas)
  - [4.8 Contratos HTTP](#48-contratos-http)
- [5. O quadro das decisões](#5-o-quadro-das-decisões)
- [6. Uber `fx`: injeção de dependência e ciclo de vida](#6-uber-fx-injeção-de-dependência-e-ciclo-de-vida)
  - [Por que](#por-que)
  - [O custo, declarado](#o-custo-declarado)
- [7. Um source code, dois entrypoints](#7-um-source-code-dois-entrypoints)
  - [Por que o servidor mora na `main`](#por-que-o-servidor-mora-na-main)
  - [A documentação da API: swag + Swagger UI](#a-documentação-da-api-swag--swagger-ui)
  - [A rota de documentação é opcional, e desligada por padrão](#a-rota-de-documentação-é-opcional-e-desligada-por-padrão)
  - [O worker não é um lugar onde mora lógica](#o-worker-não-é-um-lugar-onde-mora-lógica)
  - [Decisões do worker que não são óbvias](#decisões-do-worker-que-não-são-óbvias)
- [8. Keycloak como IDP](#8-keycloak-como-idp)
  - [Por que](#por-que-1)
  - [Decisões que não são óbvias](#decisões-que-não-são-óbvias)
  - [O modelo de autorização do domínio](#o-modelo-de-autorização-do-domínio)
- [9. Fila: SQS com ack explícito](#9-fila-sqs-com-ack-explícito)
- [10. Postgres com pgx](#10-postgres-com-pgx)
- [11. Observabilidade: OpenTelemetry + `zap`](#11-observabilidade-opentelemetry--zap)
  - [Por que OpenTelemetry, e não o SDK de um fornecedor](#por-que-opentelemetry-e-não-o-sdk-de-um-fornecedor)
  - [Uma superfície só: `observability.Observer`](#uma-superfície-só-observabilityobserver)
  - [Todas as camadas](#todas-as-camadas)
  - [Segregação por aplicação](#segregação-por-aplicação)
  - [Exportação direta, sem coletor](#exportação-direta-sem-coletor)
  - [`zap`, e por que ele não aparece nas camadas](#zap-e-por-que-ele-não-aparece-nas-camadas)
  - [Todo erro é logado — e uma vez só](#todo-erro-é-logado--e-uma-vez-só)
  - [Correlação e desligamento gracioso](#correlação-e-desligamento-gracioso)
- [12. Erros: uma biblioteca, e `errors.Is`/`errors.As` sempre](#12-erros-uma-biblioteca-e-errorsiserrorsas-sempre)
- [13. Migrações com goose](#13-migrações-com-goose)
- [14. Testes](#14-testes)
...
- [18. Rodando](#18-rodando)
- [19. Infraestrutura: Terraform em dois workspaces](#19-infraestrutura-terraform-em-dois-workspaces)
  - [O workspace é o ambiente](#o-workspace-é-o-ambiente)
  - [As diferenças entre sandbox e prod que valem nomear](#as-diferenças-entre-sandbox-e-prod-que-valem-nomear)
  - [O que a rede garante](#o-que-a-rede-garante)
  - [Segredos não passam pelo Terraform](#segredos-não-passam-pelo-terraform)
  - [Papéis separados para server e worker](#papéis-separados-para-server-e-worker)
- [20. Borda: Cloudflare com WAF na frente do ALB](#20-borda-cloudflare-com-waf-na-frente-do-alb)
- [21. TO DO — entrega contínua](#21-to-do--entrega-contínua)
  - [1. CI nos PRs](#1-ci-nos-prs)
  - [2. Revisão obrigatória do CODEOWNER](#2-revisão-obrigatória-do-codeowner)
  - [3. Deploy automatizado por release do GitHub](#3-deploy-automatizado-por-release-do-github)
  - [4. Cloudflare Access na doc da API, se formos expor em produção](#4-cloudflare-access-na-doc-da-api-se-formos-expor-em-produção)
